...
Global $Arch = @OSArch
If FileExists(@DesktopDir & "\HANDS GUI (Data).lnk") Then
	FileDelete(@DesktopDir & "\HANDS GUI (Data).lnk")
EndIf
If $Arch = "x86" Then
	FileCopy("\\grdhd5\hands\gui\x86\HANDS GUI (Data).exe", @UserProfileDir & "\documents\hands\GUI\HANDS GUI (Data).exe", $FC_OVERWRITE + $FC_CREATEPATH)
	FileCopy("\\grdhd5\hands\gui\Readme.txt", @UserProfileDir & "\documents\hands\GUI\Readme - data.txt", $FC_OVERWRITE + $FC_CREATEPATH)
...
ElseIf $Arch = "x64" Then
	FileCopy("\\grdhd5\hands\GUI\X64\HANDS GUI (Data).exe", @UserProfileDir & "\documents\hands\GUI\HANDS GUI (Data).exe", $FC_OVERWRITE + $FC_CREATEPATH)
	FileCopy("\\grdhd5\hands\gui\Readme.txt", @UserProfileDir & "\documents\hands\GUI\Readme - data.txt", $FC_OVERWRITE + $FC_CREATEPATH)
	If Not FileExists("C:\users\" & @UserName & "\documents\hands\GUI\HANDS.jpg") Then
		FileCopy("\\grdhd5\hands\gui\HANDS.jpg", "C:\users\" & @UserName & "\documents\hands\GUI\HANDS.jpg", $FC_OVERWRITE + $FC_CREATEPATH)
	EndIf
